...
class  LouvainHarness(minProgress:Int,progressCounter:Int) {
  ...
  def run[VD: ClassTag](sc:SparkContext,graph:Graph[VD,Long]): Graph[VertexState,Long] = {
    
    var louvainGraph = LouvainCore.createLouvainGraph(graph)
    var joinFullGraph = louvainGraph
    ...
    do {
	  ...
	  if (passes > 2 && currentQ > q + 0.001 ){ 
	    ...
	    joinFullGraph = Graph(louvainGraph.vertices.union(joinFullGraph.vertices), louvainGraph.edges.union(joinFullGraph.edges)).groupEdges((attr1, attr2) => attr1 + attr2 )
		println(joinFullGraph.vertices.count)	  
	  }
	  else {
	    halt = true
	  }
	...
	louvainGraph = LouvainCore.compressGraph(louvainGraph)
	saveLevel(sc,999,q,louvainGraph)
	//finalSave(sc,level,q,louvainGraph)

	return joinFullGraph
