...
object LouvainCore {
  ...
  // aggregate the edge weights of self loops. edges with both src and dst in the same community.
    ...
    internalEdgeWeights.take(3).foreach(println)
    internalEdgeWeights.count()
    ...
    internalWeights.take(3).foreach(println)
    internalWeights.count()
    ...
    newVerts.take(3).foreach(println)
    newVerts.count()
    ...
    compressedGraph.vertices.take(3).foreach(println)
    compressedGraph.vertices.count()
